...
- Undo / Redo。
- `Ctrl/⌘ + A` による全選択。
- 選択中は選択範囲、未選択時は全図形に合わせる全体表示。

### ジオメトリ操作

- Union: 2 つ以上の選択ポリゴンを結合。
- Difference: 最初に選択した対象ポリゴンから、後続の選択ポリゴンを差し引き。
- Intersection / XOR: 選択ポリゴン同士の交差・排他的論理和。
- Knife split: 単純ポリゴンを、外周とちょうど 2 点で交差する 1 本の直線で分割。
- 変形: 選択図形の 90° 回転、左右 / 上下反転、0.5× / 2× 拡大縮小（選択範囲の中心が基準）。
- 凸包 (Convex hull): 選択図形の頂点群から凸包を生成。
- 単純化 (Simplify): Douglas-Peucker により冗長な頂点を間引き。
- 整列・分布: 複数選択図形の端揃え / 中央揃え、等間隔分布。
- 頂点編集: プロパティパネルからの頂点挿入・削除、vertex-edit ツールでの `Alt + クリック` 削除。
- 面積計算: 外周面積から穴面積を差し引いた実面積を表示。
- 計測: 周長、外接矩形 (W×H)、重心の表示。
- ジオメトリ層に、正規化、交差判定、自己交差検出、極小リング判定、計測、変形、凸包、単純化などの純粋関数を配置。

### プロジェクトと UI

- `localStorage` への自動保存。
- JSON ファイルのインポート・エクスポート。
- SVG エクスポート、面積一覧 / 頂点座標一覧の CSV エクスポート。
- 面積表示単位 (mm² / cm² / m²) の切り替え。
- ライト / ダークテーマ。
- 日本語 / 英語 UI。
- ヘッダーから開けるアプリ内マニュアル。
...
- 頂点の追加・削除は外周リングに対応します（穴リングはストア API のみ）。
- 一部のプロジェクト設定（面積表示単位を除く）は、専用 UI ではなくコードまたはインポート JSON 経由で扱います。
- DXF / DWG エクスポート、3D、BIM、バックエンド共同編集機能はありません（SVG / CSV エクスポートには対応）。
